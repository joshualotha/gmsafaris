...
## 2026-05-12 — Simplify Vehicles to Manual Status Labels

### Problem
The vehicle system accumulated auto-evaluation complexity: `checkVehicles()` (70 lines), `cancelVehicle()` (30 lines), `computeVehicleDistribution()` (18 lines), 4 distribution-dependent accessors, cancellation email template, "Check Vehicles" button, and per-vehicle cancel buttons. This made the system hard to maintain and understand. The admin needed simpler manual control.
...
| Item | Lines | Why |
|------|-------|-----|
| `JoinSafari::computeVehicleDistribution()` | ~18 | Math to distribute people across vehicles — no longer needed |
| `JoinSafariVehicle::getSeatsFilledAttribute()` | ~7 | Depended on distribution |
| `JoinSafariVehicle::getSeatsAvailableAttribute()` | ~4 | Depended on seats_filled |
| `JoinSafariVehicle::getIsFullAttribute()` | ~4 | Depended on seats_available |
| `JoinSafariVehicle::getMeetsMinimumAttribute()` | ~4 | Depended on seats_filled |
| `Admin\JoinSafariController::checkVehicles()` | ~70 | Auto-evaluation of vehicle status |
| `Admin\JoinSafariController::cancelVehicle()` | ~30 | Cancellation + email logic |
| `Mail` import from Admin controller | 1 | No longer needed |
| Route: `check-vehicles` | 1 | Replaced |
| Route: `cancel` vehicle | 1 | Replaced |
| "Check Vehicles" button in admin show | ~6 | Replaced by manual dropdown |
| Cancel buttons per vehicle in admin | ~8 | Replaced by status dropdown |
| Progress bars + Seats Filled column | ~30 | No distribution math to drive them |
| `vehicle-cancelled.blade.php` email template | 97 | No automated cancellation emails |

#### Added
| Item | Lines | Purpose |
|------|-------|---------|
| `Admin\JoinSafariController::updateVehicleStatus()` | 12 | Simple status update via dropdown |
| Route: `join-safari-vehicles.update-status` | 1 | `PATCH /join-safari-vehicles/{vehicle}/status` |
| Per-vehicle status dropdown in admin show | ~15 | Admin manually picks open/confirmed/cancelled |

### Net Impact
- **~130 lines removed, ~12 added**
- Admin now has full manual control over vehicle status
- No automated decisions — admin evaluates participants and sets status
- Vehicles still auto-create on join (unchanged)

### Files Modified (Simplification)
- `app/Models/JoinSafari.php` (removed `computeVehicleDistribution()`)
- `app/Models/JoinSafariVehicle.php` (removed 4 distribution accessors)
- `app/Http/Controllers/Admin/JoinSafariController.php` (replaced `checkVehicles()` + `cancelVehicle()` with `updateVehicleStatus()`)
- `routes/admin.php` (replaced 2 routes with 1)
- `resources/views/admin/join-safaris/show.blade.php` (simplified vehicle table)
...
## 2026-05-16 — Fix 404 Errors on Uploaded Gallery Images (public_path() Mismatch)

### Problem
When users uploaded images via the admin gallery (`GalleryController@store`), the images appeared to upload successfully but returned 404 errors when displayed on the public gallery page. Browser console showed:
```
cwT68tuo7z8ns5396oMB.jpeg:1  Failed to load resource: the server responded with a status of 404 ()
```

### Root Cause
During the cPanel restructure (2026-05-06), the `public/` directory was removed and `index.php` + `.htaccess` were moved to the project root. Apache now serves directly from the project root directory.

However, Laravel's `public_path()` helper was never updated — it still returned `{base_path}/public/`. This created a path mismatch:

1. **Upload**: [`GalleryController@store`](app/Http/Controllers/Admin/GalleryController.php:83) calls `public_path('img/gallery/full')` → writes to `{root}/public/img/gallery/full/`
2. **Display**: [`GalleryImage::getFullUrlAttribute()`](app/Models/GalleryImage.php:29) uses `asset('img/gallery/full/...')` → generates URL `https://gmsafaris.co.tz/img/gallery/full/...`
3. **Apache**: Serves `{DOCUMENT_ROOT}/img/gallery/full/...` = `{root}/img/gallery/full/...` — but the file is at `{root}/public/img/gallery/full/...` → 404

The same mismatch affected safari image uploads (hero, thumbnail, gallery) since [`Admin\SafariController`](app/Http/Controllers/Admin/SafariController.php) uses `Storage::disk('public')->store()` which writes to `storage/app/public/` — but the `.htaccess` already has a rewrite rule for `/storage/` → `storage/app/public/`, so safari images via `Storage::url()` were unaffected.

### What Changed

#### [`app/Providers/AppServiceProvider.php`](app/Providers/AppServiceProvider.php)
- Added `$this->app->instance('path.public', base_path())` in the `register()` method
- This tells Laravel that the "public path" is the project root itself, not `{root}/public/`
- All calls to `public_path()` now return `base_path()` — matching where Apache serves from
...
### Net Impact
- **Critical fix**: `public_path('img/gallery/full')` now resolves to `{root}/img/gallery/full/` (matching Apache's document root)
- All future gallery uploads will be written to the correct location and served correctly
- Also fixes `public_path()` for any other code that writes directly to the public directory
- Safari uploads (which use `Storage::disk('public')`) were already working via the `.htaccess` rewrite rule and are unaffected
- Existing uploads in `{root}/public/img/gallery/` on the live server will need to be moved to `{root}/img/gallery/` (or the `public/` directory deleted and re-uploaded)

### Required Action on Live Server
After `git pull`, clear config cache:
```bash
php artisan config:clear
```
And move any existing uploads from the wrong directory:
```bash
# If public/img/gallery/ exists with uploaded files:
mv /home/gmsafari/public_html/public/img/gallery/* /home/gmsafari/public_html/img/gallery/ 2>/dev/null
rm -rf /home/gmsafari/public_html/public/
```

### Files Modified
- `app/Providers/AppServiceProvider.php` — Added `path.public` override in `register()`

---

## 2026-05-16 — Fix v2 Deployed & Verified: usePublicPath() + Orphan Cleanup

### Deployment Results (Live Server)
```bash
cd /home/gmsafari/public_html && git pull origin main && php artisan config:clear && php cleanup_orphaned_gallery.php
```

- `git pull`: Fast-forwarded `9038a06..196bc27` — pulled `bootstrap/app.php` fix and `cleanup_orphaned_gallery.php`
- `config:clear`: Configuration cache cleared successfully
- `cleanup_orphaned_gallery.php`: Deleted 5 orphaned DB records (IDs 292-296) whose files never existed on disk

### Current State
| Metric | Before | After |
|--------|--------|-------|
| `public_path()` target | `{root}/public/` (wrong) | `{root}/` (correct, matches Apache) |
| Orphaned DB records | 5 (292-296) | 0 |
| Gallery 404 errors | 5 per page load | 0 |
| New upload destination | ❌ `public/img/gallery/` | ✅ `img/gallery/` |

### Verification
The gallery page should no longer show 404 errors for those 5 filenames. Re-uploading the images through the admin gallery (`/admin/gallery`) will write files to the correct `img/gallery/full/` and `img/gallery/thumb/` directories, and they will be served correctly by Apache.

### Files Modified (Final)
- `bootstrap/app.php` — Added `$app->usePublicPath($app->basePath())` (canonical fix)
- `app/Providers/AppServiceProvider.php` — Kept `instance('path.public', ...)` as redundant safety net
- `diagnose_gallery.php` — Created (diagnostic, confirmed 5 missing files)
- `cleanup_orphaned_gallery.php` — Created (deleted 5 orphaned records)


---

## 2026-05-16 — Draft/Publish Feature for Blogs & Safaris

### Problem
Blog posts had `is_published` + `published_at` but the `update()` method wasn't auto-setting `published_at` when publishing. Safaris had only `is_active` (visibility toggle) with no concept of draft vs. published — meaning partially completed safaris couldn't be saved without being live on the site.

### What Changed

#### Database
- Created [`2026_05_16_000001_add_publish_fields_to_safaris_table.php`](database/migrations/2026_05_16_000001_add_publish_fields_to_safaris_table.php) — adds `is_published` (boolean, default false) and `published_at` (timestamp, nullable) to `safaris` table
- Ran migration locally; all 18 existing safaris marked as published with current timestamp

#### Modified Model
- [`app/Models/Safari.php`](app/Models/Safari.php):
  - Added `is_published` and `published_at` to `$fillable`
  - Added `is_published` => boolean, `published_at` => datetime casts
  - Added `scopePublished($query)` — filters `is_published = true`

#### Modified Admin Controllers
- [`app/Http/Controllers/Admin/SafariController.php`](app/Http/Controllers/Admin/SafariController.php):
  - `store()`: validates `is_published` (boolean) and `published_at` (nullable date); auto-sets `published_at = now()` when publishing
  - `update()`: same validation + auto-set logic
  - Added `togglePublish(Safari)` method — toggles `is_published`, sets `published_at` on publish
- [`app/Http/Controllers/Admin/BlogPostController.php`](app/Http/Controllers/Admin/BlogPostController.php):
  - `update()`: now auto-sets `published_at = now()` when publishing (was only in `store()` before)

#### Modified Public Controllers (all now filter by `published()` scope)
- [`app/Http/Controllers/SafariController.php`](app/Http/Controllers/SafariController.php): `index()`, `search()`, `show()`, related safaris queries, durations query
- [`app/Http/Controllers/BlogController.php`](app/Http/Controllers/BlogController.php): related safaris query
- [`app/Http/Controllers/BookingController.php`](app/Http/Controllers/BookingController.php): `create()` safari listings + selected safari lookup
- [`app/Http/Controllers/DestinationController.php`](app/Http/Controllers/DestinationController.php): related safaris query
- [`app/Http/Controllers/InquiryController.php`](app/Http/Controllers/InquiryController.php): `create()` safari listings + selected safari lookup
- [`app/Http/Controllers/SitemapController.php`](app/Http/Controllers/SitemapController.php): dynamic safari URLs

#### Modified Admin Routes
- [`routes/admin.php`](routes/admin.php): Added `PATCH /admin/safaris/{safari}/toggle-publish` → `safaris.toggle-publish`

#### Modified Admin Views
- [`resources/views/admin/safaris/index.blade.php`](resources/views/admin/safaris/index.blade.php): Added "Published" column with toggle button (green "Published" / grey "Draft")
- [`resources/views/admin/safaris/create.blade.php`](resources/views/admin/safaris/create.blade.php): Added "Publish immediately" checkbox (uncheck = save as draft)
- [`resources/views/admin/safaris/edit.blade.php`](resources/views/admin/safaris/edit.blade.php): Added "Published" checkbox with "Uncheck to save as draft" hint

### Expected Impact
- **Safari drafts**: Admin can now create/edit safaris and save them as drafts (unpublished) — they won't appear on the public site, sitemap, booking form, or inquiry form
- **Blog drafts**: `update()` now correctly auto-sets `published_at` when changing from draft to published
- **Zero data loss**: All existing safaris automatically marked as published
- **Frontend safety**: All 6 public-facing controllers now filter safaris by `published()` scope — drafts are invisible to visitors

### Files Created
- [`database/migrations/2026_05_16_000001_add_publish_fields_to_safaris_table.php`](database/migrations/2026_05_16_000001_add_publish_fields_to_safaris_table.php)

### Files Modified
- `app/Models/Safari.php`
- `app/Http/Controllers/Admin/SafariController.php`
- `app/Http/Controllers/Admin/BlogPostController.php`
- `app/Http/Controllers/SafariController.php`
...
- `routes/admin.php`
- `resources/views/admin/safaris/index.blade.php`
- `resources/views/admin/safaris/create.blade.php`
- `resources/views/admin/safaris/edit.blade.php`

---

## 2026-05-16 — Remove Em Dash (" — ") from All Content to Reduce AI-Like Tone

### Problem
Em dashes (" — ") appeared extensively throughout Blade templates and blog post database content. This punctuation pattern is strongly associated with AI-generated text, making the site's content feel inauthentic and robotic to both users and search engines.

### What Changed

#### Blade Templates (27 instances across 17 files)
Each em dash was replaced with context-appropriate natural alternatives:
- `—` → `:` (colon) — for list introductions
- `—` → `,` (comma) — for appositives/descriptions
- `—` → `;` (semicolon) — for related clauses in join-safari comparison table
- `—` → `.` (period) — for separate statements

Files modified:
- `resources/views/kilimanjaroroutes.blade.php` — 1 instance (meta description)
- `resources/views/service.blade.php` — 1 instance (meta description)
- `resources/views/destinations.blade.php` — 4 instances (meta description, alt text, body copy)
- `resources/views/safaris.blade.php` — 2 instances (meta description, body copy)
- `resources/views/about.blade.php` — 5 instances (meta description ×2, og_description, twitter_description, body copy)
- `resources/views/besttimetovisit.blade.php` — 1 instance (budget travel tip)
- `resources/views/blog-detail.blade.php` — 1 instance (author bio)
- `resources/views/layouts/app.blade.php` — 1 instance (HTML comment)
- `resources/views/luxurysafari.blade.php` — 1 instance (meta description)
- `resources/views/zanzibarbeachholiday.blade.php` — 1 instance (meta description)
- `resources/views/admin/blog/create.blade.php` — 1 instance (Rich text editor label)
- `resources/views/admin/blog/edit.blade.php` — 1 instance (Rich text editor label)
- `resources/views/admin/gallery/create.blade.php` — 1 instance (upload form hint)
- `resources/views/admin/join-safaris/create.blade.php` — 1 instance (vehicle capacity note)
- `resources/views/emails/admin/booking.blade.php` — 1 instance (email subject line)
- `resources/views/join-safari/show.blade.php` — 2 instances (form help text, info alert)
- `resources/views/join-safari/index.blade.php` — 8 instances (comparison table cells)

#### Blog Post Database Content (153 instances across 9 posts)
All `—` characters in `content` and `excerpt` columns replaced via SQLite direct update:
- ID 6 — 16 replacements
- ID 7 — 5 replacements
- ID 8 — 17 replacements
- ID 9 — 12 replacements
- ID 10 — 13 replacements
- ID 14 — 38 content + 1 excerpt
- ID 15 — 16 content + 1 excerpt
- ID 16 — 31 content + 1 excerpt
- ID 17 — 2 replacements

### Verification
- Blade templates: `grep` search confirms 0 remaining em dashes
- Database: SQL query confirms 0 blog posts with em dashes remaining

### Expected Impact
- Content reads more naturally and conversationally
- Reduced "AI-generated" perception from users and search engines
- Improved E-E-A-T perception
- 180 total em dash replacements (27 Blade + 153 database)
